...
static void handleCommand ( struct android_app *pApp, int32_t cmd ){
   ...
   switch ( cmd ){
      ...
      case APP_CMD_INIT_WINDOW:              // 1
         ...
         if ( OMGame::StartUp ( context ) != GL_TRUE ){
                ...
                case AINPUT_SOURCE_TOUCHSCREEN:
                    ...
                    switch(action){
                        case AMOTION_EVENT_ACTION_DOWN:
                           if(context->touchFunc != NULL) context->touchFunc((int)TouchState::BEGIN, AMotionEvent_getX(event, 0), AMotionEvent_getY(event, 0));
                        break;
                        case AMOTION_EVENT_ACTION_UP:
                           if(context->touchFunc != NULL) context->touchFunc((int)TouchState::ENDED, AMotionEvent_getX(event, 0), AMotionEvent_getY(event, 0));
                        break;
                        case AMOTION_EVENT_ACTION_MOVE:
                           if(context->touchFunc != NULL) context->touchFunc((int)TouchState::MOVED, AMotionEvent_getX(event, 0), AMotionEvent_getY(event, 0));
                        break;
                    }
                break;
            } // end switch
