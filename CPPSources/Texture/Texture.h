...
struct Texture{
    ...
    static std::shared_ptr<Texture> load(const char *filename, unsigned int target);
...
private:
    Texture(const char* filename,unsigned int target = GL_TEXTURE_2D, unsigned int texelType = GL_UNSIGNED_BYTE, unsigned char byte = 4, unsigned int format = GL_RGBA, unsigned int ID = 0);
};
