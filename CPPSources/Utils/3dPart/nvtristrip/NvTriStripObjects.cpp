									 ...
									 const unsigned short maxIndex)
{
	...
	{	
		...
		else 
		{
			...
			{
				logMessage("BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences\n");
			}
			else
			{
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
	...
	if (fv0 != v0 && fv0 != v1){
		if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
			logMessage("GetNextIndex: Triangle doesn't have all of its vertices\n");
			logMessage("GetNextIndex: Duplicate triangle probably got us derailed\n");
		}
		return fv0;
	}
	if (fv1 != v0 && fv1 != v1){
